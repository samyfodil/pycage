...
func instantiateGraph(ctx context.Context, r wazy.Runtime, comp *binary.Component, componentBytes []byte, cfg *config) (*Instance, error) {
	...
	for k, ci := range comp.CoreInstances {
		...
		case 0x00: // instantiate a real embedded core module
			...
			// A core instantiation's `with` arguments are local to that
			// instantiation. Resolve them directly instead of relying on the
			// graph-wide consumer-name aliases, which may legitimately collide
			// when one source is regrouped under multiple names.
			moduleCtx := instCtx
			if len(ci.Args) > 0 {
				args := make(map[string]api.Module, len(ci.Args))
				for _, arg := range ci.Args {
					provider, ok := instMods[int(arg.InstanceIdx)]
					if !ok {
						return fail(fmt.Errorf("component/instance: core module %d argument %q targets core instance %d, which was not instantiated", ci.ModuleIdx, arg.Name, arg.InstanceIdx))
					}
					args[arg.Name] = provider
				}
				moduleCtx = api.WithImportResolver(instCtx, func(name string) api.Module {
					if provider := args[name]; provider != nil {
						return provider
					}
					if resolver, ok := instCtx.Value(expctxkeys.ImportResolverKey{}).(api.ImportResolver); ok {
						return resolver(name)
					}
					return nil
				})
			}
			mod, err := instantiateCoreModule(moduleCtx, r, cfg, coreBytes, wazy.NewModuleConfig().WithName("").WithStartFunctions())
			if err != nil {
				return fail(fmt.Errorf("component/instance: instantiate core module %d (key %q): %w", ci.ModuleIdx, key, err))
			}
			instMods[k] = mod
			keyToInst[key] = mod
			keyToInst[coreInstanceResolverKey(k)] = mod
		...
		case 0x01: // inline exports: a passthrough shim regrouping earlier items
			...
			for _, e := range ci.Exports {
				...
				item, wasiCall, privMod, err := resolveInlineExportItem(ctx, r, comp, cfg, resources, in, e, coreMemSpace, coreTableSpace, coreGlobalSpace, instMods, keys, neededTypes, key, nextPrivateName, coreFuncTarget, coreMemTarget, coreTableTarget)
				if err != nil {
...
func resolveInlineExportItem(
	...
	e binary.CoreInlineExport, coreMemSpace, coreTableSpace, coreGlobalSpace []aliasTarget, instMods map[int]api.Module, _ []string,
	...
	case 0x00: // func
		...
		case binary.CoreFuncFromAlias:
			...
			def := fn.Definition()
			return shimItem{Sort: shimSortFunc, FromModule: coreInstanceResolverKey(int(al.InstanceIdx)), FromName: al.Name, ExportName: e.Name, Params: def.ParamTypes(), Results: def.ResultTypes()}, "", nil, nil
	...
	case 0x02: // memory
		...
		return shimItem{Sort: shimSortMemory, FromModule: coreInstanceResolverKey(int(at.instIdx)), FromName: at.name, ExportName: e.Name}, "", nil, nil
	...
	case 0x01: // table
		...
		return shimItem{Sort: shimSortTable, FromModule: coreInstanceResolverKey(int(at.instIdx)), FromName: at.name, ExportName: e.Name}, "", nil, nil
	...
	case 0x03: // global
		...
		global := mod.ExportedGlobal(at.name)
		if global == nil {
			return shimItem{}, "", nil, fmt.Errorf("inline export %q: core instance %d has no exported global %q", e.Name, at.instIdx, at.name)
		}
		_, mutable := global.(api.MutableGlobal)
		return shimItem{Sort: shimSortGlobal, FromModule: coreInstanceResolverKey(int(at.instIdx)), FromName: at.name, ExportName: e.Name, GlobalType: global.Type(), GlobalMutable: mutable}, "", nil, nil
...
// coreInstanceResolverKey is collision-free even when one source instance is
// regrouped under more than one consumer-declared module name.
func coreInstanceResolverKey(idx int) string {
	return fmt.Sprintf("wazy:core-instance/%d", idx)
}
...
func bindInstanceExportGraph(comp *binary.Component, exp binary.Export, componentFunc func(uint32) (bool, int, aliasTarget, error), coreFuncTarget func(int) (api.Module, string, error), resolve abi.Resolver, exports map[string]*boundExport, abiCache *CompileCache) error {
	...
	// Exporting an instance appends an alias to the component instance index
	// space. Account for those entries when translating the binary-space index
	// back to comp.Instances, which stores only actual instantiations.
	exportedInstanceAliasesBefore := 0
	for _, other := range comp.Exports {
		if other.ExternType == 0x05 && other.ExternIndex < exp.ExternIndex {
			exportedInstanceAliasesBefore++
		}
	}
	localIdx := int(exp.ExternIndex) - numImportedInstances - exportedInstanceAliasesBefore
	if localIdx < 0 {
